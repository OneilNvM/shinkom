//! Module contains functions for calculating the compatibility scores for web features.
use std::collections::HashMap;

#[cfg(feature = "hints")]
use shinkore_hints::HintEngine;

use crate::{
    constants::{MAX_COMPAT_SCORE, MAX_STATUS_COMPAT_SCORE, MAX_SUM_BROWSER_SUPPORT_COMPAT_SCORES},
    errors::CheckError,
    version::{Version, VersionRequirement},
};

use shinkore_types::{
    prelude::{
        BrowserData, BrowserDataParamType, BrowserResult, BrowserSupportContext,
        BrowserUsageContext, CompatType, LookupResults, LookupType, Scores, SupportDetailContext,
        WebFeatureContext,
    },
    schema::{Status, SupportData, VersionValue},
};

/// Calculates the compatibility score for a web feature.
///
/// Performs calculations for a global attribute or an element/ local attribute.
///
/// ## Errors
/// A [`CheckError`] is returned if any errors occurr in calculations.
pub fn calculate_compat_score(
    ctx: WebFeatureContext,
    results: &mut Vec<LookupResults>,
    browser_data_params: &Vec<BrowserDataParamType>,
) -> Result<(), CheckError> {
    let mut browser_results: Vec<BrowserResult> = vec![];
    let mut compat_score = 0.0;
    match ctx.compat_type {
        CompatType::Feature(feat) => {
            let mut browser_score = calculate_browser_score(
                ctx.name.clone(),
                CompatType::Feature(feat),
                &mut browser_results,
                browser_data_params,
            )?;

            // LookupType is used for returning the appropriate error message
            let status_score = match ctx.lookup_type {
                LookupType::Feature(name) => calculate_status_score(
                    ctx.name.clone(),
                    &feat.compat.status,
                    LookupType::Feature(name),
                )?,
                LookupType::Attribute(name) => calculate_status_score(
                    ctx.name.clone(),
                    &feat.compat.status,
                    LookupType::Attribute(name),
                )?,
            };

            #[cfg(feature = "hints")]
            {
                if let Some(tags) = &feat.compat.tags {
                    let mut hint_engine = HintEngine::new();

                    hint_engine.tier_3_hints(tags);

                    hint_engine.log_hints();

                    hint_engine.clear_hints();
                }
            }

            if status_score != 0.0 {
                compat_score += ((browser_score + status_score) / MAX_COMPAT_SCORE as f32) * 100.0;
            } else {
                browser_score = 0.0;
            }

            results.push(LookupResults {
                name: ctx.name,
                mdn_url: feat.compat.mdn_url.clone(),
                compat_score: format!("{compat_score:.2}"),
                browser_score: format!(
                    "{:.2}",
                    ((browser_score / MAX_SUM_BROWSER_SUPPORT_COMPAT_SCORES as f32) * 100.0)
                ),
                status_score: format!(
                    "{:.1}",
                    (status_score / MAX_STATUS_COMPAT_SCORE as f32) * 100.0
                ),
                browsers: Some(browser_results),
            });

            Ok(())
        }
        CompatType::GlobalAttributes(g_attrib) => {
            let mut browser_score = calculate_browser_score(
                ctx.name.clone(),
                CompatType::GlobalAttributes(g_attrib),
                &mut browser_results,
                browser_data_params,
            )?;
            let status_score = calculate_status_score(
                ctx.name.clone(),
                &g_attrib.compat.status,
                LookupType::Attribute(&ctx.name),
            )?;

            #[cfg(feature = "hints")]
            {
                if let Some(tags) = &g_attrib.compat.tags {
                    let mut hint_engine = HintEngine::new();

                    hint_engine.tier_3_hints(tags);

                    hint_engine.log_hints();

                    hint_engine.clear_hints();
                }
            }

            if status_score != 0.0 {
                compat_score += ((browser_score + status_score) / MAX_COMPAT_SCORE as f32) * 100.0;
            } else {
                browser_score = 0.0;
            }

            results.push(LookupResults {
                name: ctx.name,
                mdn_url: g_attrib.compat.mdn_url.clone(),
                compat_score: format!("{compat_score:.2}"),
                browser_score: format!(
                    "{:.2}",
                    ((browser_score / MAX_SUM_BROWSER_SUPPORT_COMPAT_SCORES as f32) * 100.0)
                ),
                status_score: format!(
                    "{:.1}",
                    (status_score / MAX_STATUS_COMPAT_SCORE as f32) * 100.0
                ),
                browsers: Some(browser_results),
            });

            Ok(())
        }
    }
}

/// Calculates the status score for a web feature
///
/// Returns the score as an [`f32`].
///
/// ## Errors
/// A [`CheckError`] is returned if the status is not available for the feature.
pub fn calculate_status_score(
    feature_name: String,
    compat_status: &Option<Status>,
    lookup_type: LookupType,
) -> Result<f32, CheckError> {
    let mut status_score = 0.0;
    if let Some(status) = compat_status {
        if status.standard_track {
            if status.deprecated {
                status_score = 0.0;
            } else if status.experimental {
                status_score = 50.0;
            } else {
                status_score = 100.0;
            }
        } else if status.experimental {
            status_score = 10.0
        } else if status.deprecated {
            status_score = 0.0;
        }

        #[cfg(feature = "hints")]
        {
            use shinkore_types::schema::StatusIssue;

            if status_score < 100.0 {
                let mut hint_engine = HintEngine::new();

                hint_engine.compile_status_hints(StatusIssue {
                    feature_name,
                    status,
                });

                hint_engine.log_hints();

                hint_engine.clear_hints();
            }
        }
    } else {
        match lookup_type {
            LookupType::Feature(tag) => {
                return Err(CheckError::MissingStatus(format!("tag <{tag}>")));
            }
            LookupType::Attribute(attribute) => {
                return Err(CheckError::MissingStatus(format!(
                    "local attribute '{attribute}'"
                )));
            }
        }
    }

    Ok(status_score)
}

/// Calculates the browser score for a web feature.
///
/// Returns the total browser score between the available browsers.
///
/// ## Errors
/// A [`CheckError`] is returned if an error occurs in the calculations for support scores.
pub fn calculate_browser_score(
    feature_name: String,
    compat_type: CompatType,
    browser_results: &mut Vec<BrowserResult>,
    browser_data_params: &Vec<BrowserDataParamType>,
) -> Result<f32, CheckError> {
    let mut browser_score_total: f32 = 0.0;

    match compat_type {
        CompatType::Feature(el) => {
            for (browser_name, support) in &el.compat.support {
                browser_score_total += calculate_support(
                    BrowserSupportContext {
                        feature_name: &feature_name,
                        compat: &el.compat,
                        browser_name,
                        support,
                    },
                    browser_data_params,
                    browser_results,
                )?;
            }
        }
        CompatType::GlobalAttributes(g_attrib) => {
            for (browser_name, support) in &g_attrib.compat.support {
                browser_score_total += calculate_support(
                    BrowserSupportContext {
                        feature_name: &feature_name,
                        compat: &g_attrib.compat,
                        browser_name,
                        support,
                    },
                    browser_data_params,
                    browser_results,
                )?;
            }
        }
    }

    Ok(browser_score_total)
}

/// Calculates the support score for the web feature on a specific browser.
///
/// Browser name must be present in `browser_data` or one of the `proxied` browser names.
///
/// ## Errors
/// A [`CheckError`] is returned if the required browser data parameters are not provided.
fn calculate_support(
    ctx: BrowserSupportContext,
    browser_data_params: &Vec<BrowserDataParamType>,
    browser_results: &mut Vec<BrowserResult>,
) -> Result<f32, CheckError> {
    let mut browser_data = None;
    let mut usage_data = None;
    let mut browser_score: f32 = 0.0;

    // Proxy browsers that have no usage data to browsers with similar engines.
    let browser_proxies = HashMap::from([
        (String::from("oculus"), String::from("chrome")),
        (
            String::from("opera_android"),
            String::from("chrome_android"),
        ),
        (String::from("webview_ios"), String::from("safari_ios")),
    ]);

    // Retrieve data from parameter types
    for param_type in browser_data_params {
        match param_type {
            BrowserDataParamType::BrowserData(data) => browser_data = Some(data),
            BrowserDataParamType::UsageData(data) => usage_data = Some(data),
        }
    }

    if browser_data.is_none() || usage_data.is_none() {
        return Err(CheckError::WrongBrowserDataParams(
            String::from("BrowserData and UsageData"),
            format!(
                "BrowserData is {:?} and UsageData is {:?}",
                browser_data.is_some(),
                usage_data.is_some()
            ),
        ));
    }

    let mut raw_score_val: f32 = 0.0;

    match ctx.support {
        SupportData::Single(detail) => {
            // Calculate browser_score for version_added
            match_version_added(
                SupportDetailContext {
                    browser_name: ctx.browser_name,
                    detail,
                },
                &mut browser_score,
                browser_data.unwrap(),
            )?;

            if detail.partial_implementation {
                browser_score *= 0.2;
            }

            // Get the raw score before the weighting calculation
            let raw_score = format!("{:.2}", browser_score);
            raw_score_val = browser_score;

            if browser_data
                .unwrap()
                .browsers
                .contains_key(ctx.browser_name)
            {
                // Calculate the proxy weight if the browser name is proxied
                if browser_proxies.contains_key(ctx.browser_name) {
                    let proxy_name = browser_proxies.get(ctx.browser_name).unwrap();

                    calculate_weight(
                        BrowserUsageContext {
                            browser_name: proxy_name,
                            usage_data: usage_data.unwrap(),
                        },
                        true,
                        &mut browser_score,
                    );
                } else {
                    calculate_weight(
                        BrowserUsageContext {
                            browser_name: ctx.browser_name,
                            usage_data: usage_data.unwrap(),
                        },
                        false,
                        &mut browser_score,
                    );
                }
            }

            browser_results.push(BrowserResult {
                browser_name: ctx.browser_name.to_owned(),
                score: Scores {
                    raw_score,
                    weighted_score: format!("{:.2}", browser_score),
                },
                versions: Some(SupportData::Single(detail.clone())),
            })
        }
        SupportData::Multiple(details) => {
            // Separate the total weighted score from the total raw score
            let mut sum_of_raw_scores = 0.0;
            let mut sum_of_weighted_scores = 0.0;
            let mut skip = false;

            for detail in details {
                let mut support_score = 0.0;

                // Use support_score instead of browser_score for average calculations later
                match_version_added(
                    SupportDetailContext {
                        browser_name: ctx.browser_name,
                        detail,
                    },
                    &mut support_score,
                    browser_data.unwrap(),
                )?;

                if !skip {
                    if detail.partial_implementation {
                        support_score *= 0.2;
                    }
                    skip = true;
                }

                sum_of_raw_scores += support_score;

                if browser_data
                    .unwrap()
                    .browsers
                    .contains_key(ctx.browser_name)
                {
                    // Calculate the proxy weight if the browser name is proxied
                    if browser_proxies.contains_key(ctx.browser_name) {
                        let proxy_name = browser_proxies.get(ctx.browser_name).unwrap();

                        calculate_weight(
                            BrowserUsageContext {
                                browser_name: proxy_name,
                                usage_data: usage_data.unwrap(),
                            },
                            true,
                            &mut support_score,
                        );
                    } else {
                        calculate_weight(
                            BrowserUsageContext {
                                browser_name: ctx.browser_name,
                                usage_data: usage_data.unwrap(),
                            },
                            false,
                            &mut support_score,
                        );
                    }
                }

                sum_of_weighted_scores += support_score
            }

            // Calculate the averages for the raw score and weighted score
            sum_of_raw_scores /= details.len() as f32;
            browser_score = sum_of_weighted_scores / details.len() as f32;

            raw_score_val = sum_of_raw_scores;

            browser_results.push(BrowserResult {
                browser_name: ctx.browser_name.to_owned(),
                score: Scores {
                    raw_score: format!("{:.2}", sum_of_raw_scores),
                    weighted_score: format!("{:.2}", browser_score),
                },
                versions: Some(SupportData::Multiple(details.clone())),
            })
        }
        SupportData::Simple(version) => match version {
            VersionValue::Version(_) => {
                browser_score = 100.0;
                raw_score_val = browser_score;
            }
            VersionValue::IsSupported(supported) if *supported => {
                browser_score = 100.0;
                raw_score_val = browser_score;
            }
            _ => (),
        },
        _ => (),
    }

    #[cfg(feature = "hints")]
    {
        use shinkore_types::schema::BrowserIssue;

        if ctx.browser_name != "ie" && raw_score_val < 100.0 {
            let mut hint_engine = HintEngine::new();

            hint_engine.compile_browser_hints(BrowserIssue {
                feature_name: ctx.feature_name.clone(),
                browser_target: ctx.browser_name.to_owned(),
                compat: ctx.compat,
                support: ctx.support,
            });

            hint_engine.log_hints();

            hint_engine.clear_hints();
        }
    }

    Ok(browser_score)
}

/// Calculates the weighted score for an individual browser.
///
/// The weight is calculated by summing the total usage between a browser's versions and dividing it by
/// the total market share. This is then multiplied with the raw browser score giving the weighted score,
/// based on browser usage.
///
/// A higher weighted score **(e.g. 20)** means a feature is more likely to affect a lot of users on a browser, meanwhile
/// a low score **(e.g. 0.2)** means that it won't affect many users.
fn calculate_weight(ctx: BrowserUsageContext, proxied: bool, browser_score: &mut f32) {
    if let Some(usage) = ctx.usage_data.agents.get(ctx.browser_name) {
        // Set total usage to 0 if the browser is proxied
        let total_usage = if proxied {
            0.0
        } else {
            usage.values().sum::<f32>()
        };

        *browser_score *= total_usage / ctx.usage_data.market_share;
    }
}

/// Matches the [`VersionValue`] variant of the `version_added` field.
fn match_version_added(
    ctx: SupportDetailContext,
    browser_score: &mut f32,
    browser_data: &BrowserData,
) -> Result<(), CheckError> {
    match &ctx.detail.version_added {
        VersionValue::Version(added_version) => {
            if let Some(browser) = browser_data.browsers.get(ctx.browser_name)
                && let Some(current_version) = &browser.values().next().unwrap().engine_version
            {
                let target: Version = current_version.parse()?;
                let requirement: VersionRequirement = added_version.parse()?;
                *browser_score = decay_score(target, requirement, 10) as f32;
            }
        }
        VersionValue::IsSupported(supported) => {
            if *supported {
                *browser_score = 100.0;
            } else {
                *browser_score = 0.0;
            }
        }
        _ => (),
    }

    Ok(())
}

fn decay_score(target: Version, added: VersionRequirement, decay_window: u32) -> u8 {
    if !added.is_satisfied_by(&target) {
        return 0;
    }

    let version_added = match added {
        VersionRequirement::GreaterThanOrEqualTo(version) => version,
        VersionRequirement::LessThanOrEqualTo(version) => version,
        VersionRequirement::Range(min_version, _max_version) => min_version,
    };

    let version_safety = target.major_versions_ahead(&version_added);

    let raw_score = (version_safety as f32 / decay_window as f32) * 100.0;

    raw_score.clamp(0.0, 100.0) as u8
}

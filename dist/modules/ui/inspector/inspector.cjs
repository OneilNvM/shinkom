...
* @extends {UIComponent}
*/
var CompatInspector = class CompatInspector extends require_core_ui_component.UIComponent {
	...
	*/
	static register() {
		if ("customElements" in globalThis && !globalThis.customElements.get("sk-compat-inspector")) globalThis.customElements.define("sk-compat-inspector", require_core_elements_inspector_element.CompatInspectorElement);
	}
	/**
	...
	*/
	#handleToggleFreeze = (e) => {
		if (e.composedPath().includes(this.#stateBind?.ignorePanelEl) || e.composedPath().includes(this.#stateBind?.ignoreCompatViewEl)) return;
		e.preventDefault();
		e.stopPropagation();
	...
	*/
	...
	mount() {
		...
		this.inspectorEl = document.createElement("sk-compat-inspector");
	...
	*/
	#setupGlobalListeners() {
		...
		window.addEventListener("click", this.#handleToggleFreeze, {
			signal,
			capture: true
		});
...
//#endregion
exports.CompatInspector = CompatInspector;

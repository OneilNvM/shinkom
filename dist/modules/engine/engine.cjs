...
const require_shinkore = require("../pkg/shinkore.cjs");
const require_browser_data = require("../gen/browser-data.cjs");
const require_browser_usage_data = require("../gen/browser-usage-data.cjs");
	...
	async loadWasm(wasmURL = void 0) {
		...
		try {
			if (isNode) {
				...
				let wasmPath = await require_helpers.getModulePath("shinkom/wasm");
				let wasmBuffer;
				if (wasmPath.toString().endsWith("shinkore_bg.wasm")) {
					...
					this.#wasmLoaded = (async () => {
						await require_shinkore.__wbg_init({ module_or_path: wasmBuffer });
					})();
				} else throw new Error("Path does not lead to WASM file.");
			} else if (wasmURL) this.#wasmLoaded = (async () => {
				await require_shinkore.__wbg_init({ module_or_path: wasmURL });
			})();
			else this.#wasmLoaded = (async () => {
				await require_shinkore.__wbg_init();
			})();

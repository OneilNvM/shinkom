...
export class CompatEngine {
    ...
    constructor(bcd_html_data, bcd_svg_data, bcd_browser_data, ciu_usage_data) {
        const ret = wasm.compatengine_new(bcd_html_data, bcd_svg_data, bcd_browser_data, ciu_usage_data);
        this.__wbg_ptr = ret;
...
const CompatEngineFinalization = (typeof FinalizationRegistry === 'undefined')
    ? { register: () => {}, unregister: () => {} }
    : new FinalizationRegistry(ptr => wasm.__wbg_compatengine_free(ptr, 1));
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('shinkore_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
